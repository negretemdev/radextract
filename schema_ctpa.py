...
- All pe_ fields must be null (present null, evidence null) unless pulmonary_embolism is true. When it is true: pe_saddle, pe_main, pe_lobar (interlobar counts as lobar), pe_segmental and pe_subsegmental are the arterial levels, true for each level described and false for a level explicitly negated; pe_right and pe_left are the sides involved ("bilateral" makes both true), false only when the report explicitly says that side is clear or free of thrombus; pe_multiple is true when more than one embolus, filling defect or vessel is involved and false for a single one; a clot described as occlusive makes pe_occlusive true and pe_nonocclusive false, a clot described as nonocclusive the reverse, and both are true when both are described.
