...
- System One (2026-09-22): Jev key works but Vercel returns 403 customer_verification_required until the user adds a card to the Vercel team; the Jev promotion ends 2026-09-25. OpenJev cannot run on the laptop (needs >= 24 GB NVIDIA on Linux, or Apple silicon). The system-one-adapter package was removed after the auto-mode check blocked integrating it (it pulls TypeSafe's cloud SDK and an unfamiliar `httpx2`); its prompts are reproduced in `decide.py`.
