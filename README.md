...
- `pe_multiple` counts emboli, not vessels: present for plural emboli, "multiple" or a count above one; mentioned but not present for "a single" embolus; unmentioned for one thrombus extending through several vessels. The lingula is part of the left upper lobe, so a lingular artery sets both `pe_segmental_lingula` and `pe_segmental_left_upper`.
